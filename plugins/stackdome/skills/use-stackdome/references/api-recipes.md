...
Preview deletion has a different terminal condition: an accepted delete may first return the retained preview ID in `Deleting`. Primary proof is a bounded poll of its detail GET until that request returns `404`. If detail polling cannot provide that proof and the list is used instead, fetch every page from `1` through the response's current `total_pages`, following its `page`/`page_size` metadata. If pagination metadata changes while traversing the set, restart within a fixed deadline and maximum number of attempts. Only absence of the exact retained ID across one complete, stable page set proves deletion; otherwise report deletion as unverified. Absence from one page is insufficient. Do not require `Ready` or report deletion from `202`, `204`, or `Deleting` alone. After deleting a configuration, require its detail GET to return `404`; if list absence is also reported, exhaust its paginated configuration list by the same rule.
